...
class RobotiqGripperNode:
    def __init__(self):
        rospy.init_node('robotiq_gripper_node_1')
        self.address = rospy.get_param('~gripper_ip', '10.1.1.3')
        self.port = rospy.get_param('~gripper_port', 63352)
        self.s = None
        
        self.connect()
        
        self.close_sub = rospy.Subscriber('/gripper1_close', Bool, self.close_callback)
        self.open_sub = rospy.Subscriber('/gripper1_open', Bool, self.open_callback)
        self.action_completed_pub = rospy.Publisher('/gripper1_action_completed', Bool, queue_size=10)
