...
class UR1RobotNode:
    def __init__(self):
        ...
        self.selection_vector = [1, 0, 0, 0, 0, 0]  # Only Z-axis force
        self.wrench_down = [0, 0, 50, 0, 0, 0]  # 50N upward force
        self.force_type = 2
        self.limits = [2, 2, 1.5, 1, 1, 1]
    ...
    def force_move_down(self, duration=2.0):
        """Execute force control movement downward"""
        rate = rospy.Rate(500)  # 500Hz control loop
        start_time = time.time()
        
        self.rtde_c.forceMode(self.task_frame, self.selection_vector, self.wrench_down, self.force_type, self.limits)
    ...
    def move_callback(self, pose_msg):
        ...
        try:
            success = False
            if pose_msg.pose.position.y == 1.0:  # Joint movement flag
                joint_angle = pose_msg.pose.position.x
                current_joints = self.rtde_r.getActualQ()
                target_joints = current_joints[:]
                target_joints[0] = joint_angle
                success = self.rtde_c.moveJ(target_joints, 0.5, 0.5)
            elif pose_msg.pose.position.y == 2.0:  # Force move flag
                success = self.force_move_down()
            else:
